...
/**
 * Provides centralized logic for updating laundry machine UI components.
 * <p>
 * This class handles:
...
public class MachineStateHelper {
    ...
    /**
     * Toggles the selection outline of a machine view.
    ...
    /**
     * Updates a machine's number label, status text, and background color.
     * <p>
     * Uses dynamic resource lookup via getIdentifier() to find sub-views,
     * allowing it to work with any layout following the "machine_number_X"
     * and "machine_status_X" naming convention.
     *
     ...
     * @param state      the state constant (e.g., {@link #STATE_OPEN}, {@link #STATE_IN_USE})
     */
    public static void setMachineState(View view, int machineNum, int state) {
        if (view == null) return;

        // Dynamically resolve view IDs based on machine number to avoid hardcoding
        ...
        // Update the machine number label
        if (tvNumber != null) {
            tvNumber.setText(String.valueOf(machineNum));
        }

        // Update the status label and background color
        if (tvStatus != null) {
            String statusText;
            int bgColorRes;
            ...
            // Apply text and background color updates
